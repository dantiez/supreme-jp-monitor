...
- **A scan takes about 4 seconds, and used to take two minutes.** Measured, not guessed: one round trip to Neon costs 80ms from here, and the old code wrote one row per statement -- 268 products plus 996 sizes plus transaction overhead is roughly 1,800 round trips, about 144 seconds. Reading Supreme took 2. So 98% of the wait was write latency on rows nothing was waiting for individually. The writes now go in bulk via `unnest`, in one transaction for the whole scan; same upserts, same semantics, a handful of round trips. Verified end to end: run #37 took 121s, run #38 took 3s, both 268 products, and the watch-list carry-forward still produced the one genuine sell-out.
- **One transaction for the whole scan, not one per product.** A failure halfway through used to leave half the catalogue updated against a baseline that had already moved.
